...
独立授权的批准人检查准确版本和校验和。批准设置 `approved_version_id`；拒绝或要求修改会把资产返回草稿。后续任何编辑都会使活动审批失效。

### 6.5 归档

授权用户使用原因归档。归档会清除活动批准指针，但保留版本、决定、运行、引用和审计历史。恢复会返回工作状态，但不会恢复审批。
