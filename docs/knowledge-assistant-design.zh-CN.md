...
认证后的每个工作台页面右侧都会显示知识助手入口。入口可以上下拖动，并把位置保存在浏览器本地存储中。点击后打开响应式抽屉，包含英文/中文回答语言、问题输入、异步加载和安全错误状态、证据状态、回答、完整引用、可展开来源摘录、可见相似度分数、关联 ID 和耗时；手机端抽屉接近全屏。`/knowledge/assistant` 完整页面继续用于查看较长内容，并隐藏重复悬浮入口。两个界面都明确标记为只读、固定使用商用厨房智能体，并且不显示 IVC 生产选项。
