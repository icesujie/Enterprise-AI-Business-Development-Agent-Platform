...
最终业务验收需要经过批准的真实 Sari Arta 品牌指南、最终中英文内容审核、从已批准人工后继版本获得的真实人工编辑距离，以及受控 OpenAI 对比。这些工作被有意延期，并且必须在启用生产营销内容生成或自动发布之前完成。

本次延期**不会阻止** Phase 3.2.4 自然搜索与 AI 搜索技术基础：Sitemap、Robots/爬虫就绪、结构化数据、Metadata 架构、公开页面架构和搜索归因设计可以独立推进。本次延期**会阻止**营销内容智能体的生产激活、网站内容自动生成、内容自动发布、社交媒体发布，以及基于生成内容的外部营销沟通。

### Phase 3.2.4A — 自然搜索与 AI 搜索技术基础

- 为标准公开页面和内部工作区路由增加了经过审核的爬取边界。
- 增加了可维护的 Robots 和 Sitemap 策略，并明确审核 Googlebot、Bingbot 和 OAI-SearchBot。
- 增加了中英文 Canonical/Open Graph Metadata，以及 Organization、WebSite 和 Breadcrumb 结构化数据。
- 增加了可复用的未来已发布页面 Sitemap 过滤器和 Article Schema 构建器，但没有创建或发布内容。
- 增加了默认禁用的 IndexNow 就绪能力和 Google/Bing 验证配置点。
- 增加了最小化搜索获客归因，同时保留 `website` 和 `website_ai_assistant` 作为 CRM 正式录入来源。
- 生产域名验证、搜索管理平台接入、Analytics 和受治理公开内容发布仍属于未来或人工任务。
...
| 自然与 AI 搜索基础 | 公开/私有爬取策略、Canonical Metadata、结构化数据、Sitemap 治理、IndexNow 就绪和最小化获客归因 |
...
- Phase 3.2.3.5 营销内容生成评估与 UX 验证：工程实现与技术评估已完成。
- Phase 3.2 业务验收准备：已实现；业务验收待完成/已延期，生产激活已禁用。
- Phase 3.2.4A 自然搜索与 AI 搜索技术基础：已完成并验证；生产域名接入仍待完成。

营销智能体现在通过与提供方无关的运行时和现有人工审核流程，创建有引用、不可变的 `generated` 版本。生产部署、真实客户数据、真实公开知识启用、发布和外部沟通仍按适用规则保持关闭或需要人工批准。

确定性评估基线覆盖五个合成场景、全部五种支持格式和中英文配对案例。在延期的业务验收前置条件完成之前，生产营销生成以及所有生成内容发布或外部沟通能力继续保持阻止状态。

业务验收工作区是 `/marketing-content/acceptance`。品牌指南验证、最终中英文审核、真实人工编辑距离，以及一个英文和一个中文的受控 OpenAI 对比仍明确处于 pending。Phase 3.2.4A 已在不生成或发布营销内容的前提下完成，因此这些验收阻塞条件保持不变。
...
#### 进行中的 Phase 3.2.4 — 自然搜索与 AI 搜索可见性基础
...
Phase 3.2.4A 已经实现 Sitemap、爬虫就绪、结构化数据基础设施、Metadata 架构、公开页面边界和归因就绪能力，同时没有激活营销内容生成或发布。生产域名验证、受治理动态发布、搜索管理平台接入和测量仍属于后续受控工作。
