...
Phase 2.5.2 为已批准或已启用、已绑定智能体的当前版本增加明确的 **开始处理** 命令。它创建持久处理运行和按智能体隔离的 pgvector 分块，同时让本控制面继续作为审批权威来源。它仍然不实现检索或对话式知识助手。参见 `knowledge-processing-pipeline-design.zh-CN.md`。

## 11. Phase 2.5.3 治理实现

Phase 2.5.3 取代上述初始单版本治理限制。已实现控制面现在支持元数据编辑、替换版本、明确的当前/已发布/已生效指针、准确版本审核归因、独立发布、归档恢复、创建不可变后继版本的回滚、绑定禁用/重新启用、拆分知识权限，以及以文档为中心的 `knowledge_audit_logs` 时间线。

原 Phase 2.5.1 生命周期图保留为第一版实现的历史说明。当前权威生命周期和 API 矩阵见 `knowledge-governance-design.zh-CN.md`。检索和对话式回答仍保持禁用。
