...
Phase 2.6.1 enables the first read-only governed retrieval boundary through `POST /api/v1/knowledge/search`. Tenant and agent capability authorization precedes query embedding; pgvector search then requires active lifecycle, exact published/active version equality, approved review, completed processing, enabled same-agent binding, language, provider/model, and similarity eligibility. It returns evidence and citations only—no answer generation. See `knowledge-retrieval-design.en.md`.
